...
#==============================================================================
# Import 2004 - 2020 beach data from shellfish DB
#==============================================================================
...
sort(unique(beach$inactive_datetime))

# Read beach as geometry data ====================================
...
bidn_proof = beach_st %>% 
  ...
  filter(active_year == 2019 & inactive_year == 2019) %>% 
...
# Set path
flt_path = glue("C:\\data\\RStudio\\intertidal_shellfish\\Shapefiles\\FlightCounts_{current_year}\\")
...
# Get flight data
flt_obs = read_sf(glue("{flt_path}\\UClam_{current_year}.shp"))
...
flt_obs = flt_obs %>% 
  ...
  mutate(uuid = remisc::get_uuid(nrow(flt_obs))) %>% 
...
# # I was being imported as 4269, but was actually 2927
flt_zero = read_sf(glue("{flt_path}\\Zero_{current_year}.shp"), crs = 2927)
...
flt_zero = flt_zero %>% 
  ...
  mutate(uuid = remisc::get_uuid(nrow(flt_zero))) %>% 
  mutate(User_ = "S") %>% 
...
# Convert to proper crs
flt_zero = st_transform(flt_zero, crs = 2927)
...
# Combine tide times with flight obs
flt_st = flt_st %>% 
...
# RESULT: Nothing worrisome. Max dif was 90 minutes, 
...
#======================================================================
# Delete previously entered flight data for year so it can be re-entered
#======================================================================

# # Survey table ids
# qry = glue::glue("SELECT s.survey_id, s.created_datetime FROM survey AS s ",
#                  "LEFT JOIN survey_type_lut AS st ON s.survey_type_id = st.survey_type_id ",
#                  "WHERE (survey_type_description = 'Ground based, low tide harvester count, clam and oyster' ",
#                  "OR survey_type_description = 'Aerial harvester count, clam and oyster') ",
#                  "AND date_part('year', s.survey_datetime) = {year} and s.created_datetime = '2018-01-23 13:46:06.27858-08'")
# 
# # Get survey_ids
# db_con = db_connect(current_db = "shellfish")
# s_id = dbGetQuery(db_con, qry)
# dbDisconnect(db_con)
# 
# # Check created time
# unique(s_id$created_datetime)
# 
# # Create vector of survey_ids
# s_ids = paste("'", s_id$survey_id, "'", sep = "")
# s_ids = paste(s_ids, collapse = ", ")
# 
# # Get survey_detail_ids
# db_con = db_connect(current_db = "shellfish")
# se_id = dbGetQuery(db_con,
#                    glue::glue("SELECT survey_event_id FROM survey_event ",
#                               "WHERE survey_id IN ({s_ids})"))
# dbDisconnect(db_con)
# 
# # Create vector of survey_event_ids
# se_ids = paste("'", se_id$survey_event_id, "'", sep = "")
# se_ids = paste(se_ids, collapse = ", ")
# 
# # Dump from shellfish ========================================
# 
...
# dbDisconnect(db_con)
# 
# # Dump from shellfish_archive ===============================
# 
# # Dump survey_event data
# db_con = db_connect(current_db = "shellfish_archive")
# dbExecute(db_con,
#           glue::glue("DELETE FROM survey_event ",
#                      "WHERE survey_event_id IN ({se_ids})"))
# dbDisconnect(db_con)
...
#================================================================
# VERIFY SAME CONTENT IN BOTH shellfish AND shellfish_archive DBs
# One more row in archive beach_boundary_history table...Probably Potlatch DNR
#================================================================

# Get table names and row counts in source db
source_row_counts = db_row_counts(hostname = "local", dbname = "shellfish")
source_row_counts = source_row_counts %>% 
  select(table, row_count_local = row_count)

# Get table names and row counts in sink db
sink_row_counts = db_row_counts(hostname = "local", dbname = "shellfish_archive")
sink_row_counts = sink_row_counts %>% 
  select(table, row_count_archive = row_count)
...
identical(compare_counts$row_count_local, compare_counts$row_count_archive)

# Pull out cases that differ
if (!identical(compare_counts$row_count_local, compare_counts$row_count_archive)) {
  diff_counts = compare_counts %>% 
    filter(!row_count_local == row_count_archive)
}

#================================================================
# VERIFY SAME CONTENT IN BOTH shellfish local AND shellfish prod DBs
# All the same in local and prod....Needed to write beach_boundary_histery to prod in 2019...did not do it earlier
#================================================================
...
# Get table names and row counts in sink db
sink_row_counts = db_row_counts(hostname = "prod", dbname = "shellfish")
sink_row_counts = sink_row_counts %>% 
  select(table, row_count_prod = row_count)
...
# Verify that table names and row counts in source and sink db's are identical
identical(compare_counts$row_count_local, compare_counts$row_count_prod)

# Pull out cases that differ
if (!identical(compare_counts$row_count_local, compare_counts$row_count_prod)) {
  diff_counts = compare_counts %>% 
    filter(!row_count_local == row_count_prod)
}

# #================================================================
# # This year only....forgot to add beach_boundary_history to prod
# # earlier, so doing it now.....All now agree
# #================================================================
# 
# # Read
# db_con = dbConnect(odbc::odbc(), timezone = "UTC", dsn = "local_shellfish")
# dat = dbReadTable(db_con, "beach_boundary_history")
# dbDisconnect(db_con)
# 
# unique(format(dat$created_datetime, "%H:%M"))
# 
# # Select only the 2019 data....should be 189 rows
# dat = dat %>% 
#   filter(year(active_datetime) == 2019)
# 
# # Write
# db_con = pg_con_prod(dbname = "shellfish")
